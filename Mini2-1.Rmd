...
**Due Saturday, September 28 @ 11:59pm through Github**

Your next assignment is to practice the 6 `dplyr` functions so that you are comfortable using them.  See how many of these different tasks you can complete in your 30 minutes.  For the first set of tasks, I will tell you which functions to use.  For the second set of tasks, you must determine the right function yourself!

As a reminder, the 6 functions are:
...
* Use `filter()` to find all college graduates with medical expenditures about $5000.
```{r}
...
* Use `arrange()` to sort the dataset to find the oldest person.
```{r}

```

* Use `arrange()` to sort the dataset to find the smoker (`eversmk`) with the lowest BMI.
```{r}
...
* Use `mutate()` to create a new variable called `log10exp` that has an individual's log10 medical expenditures.
```{r}
...
* Use `group_by()` and `summarize()` to find the number of people, mean, and median medical expenditures for those who have and don't have a major smoking-caused disease (`mscd`).
```{r}

```

* Use `group_by()` and `summarize()` to find the number of people, mean, and median BMIs for those who have and haven't ever smoked (`eversmk`).
```{r}
...
* Compare the mean and median medical expenditures between those who have and haven't ever smoked
```{r}

```

* Compare the minimum and maximum medical expenditures between those who are poor and those who are not.
```{r}
...
* Find the number of individuals in each of the education groups.
```{r}
